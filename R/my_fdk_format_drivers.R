# NOTE: because of hardcoding in FluxDataKit the relevant function is slightly
...
#' @param ccov data frame with columns: date, ccov, sitename, source
#'
#' @return returns an rsofun compatible driver file for the provided
#'  sites
#' @export

my_fdk_format_drivers <- function(
    site_info,
  ...
  list_flux <- lapply(site_info$sitename, function(site){
    ...
    # NOTE: in FluxDataKit: ccov <- fdk_process_cloud_cover(
    # NOTE: in FluxDataKit:   path = "/data_2/FluxDataKit/FDK_inputs/cloud_cover/",
    # NOTE: in FluxDataKit:   site = site
    # NOTE: in FluxDataKit: )
    # NOTE: change from FDK
    # ccov <- readr::read_csv(here::here("data/ccov_LEMONTREE_SITES.csv")) |>
    #  select(-source)
    ...
    df_flux <- df_flux |>
      ...
      dplyr::select(
        ...
        le,
        le_qc,
        LAI    # modification of code to keep LAI
      ) |>
      dplyr::group_by(sitename) |>
      tidyr::nest()
